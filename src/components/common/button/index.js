...
const Button = styled.button`
	...
	${props => {
		switch (props.variant) {
			case 'outlined':
				return `border: 1px solid ${props.theme.color[props.color || 'default']};
                    color: ${props.theme.color[props.color || 'default']};
                    background-color: transparent;
                    &:hover {
                      background: ${props.theme.color[props.color || 'default']};
                      color: #fff;
                      transition: 0.2s all;
                    }`;
			case 'soft':
				return `background: ${props.theme.color[props.color || 'default']}64;
                    color: ${props.theme.colors.color[props.color || 'default']}64;
                    &:hover {
                      box-shadow: 0 2px 1px -1px ${props.theme.color[props.color || 'default']}33,
                                  0 1px 1px 0 ${props.theme.color[props.color || 'default']}26,
                                  0 1px 3px 0 ${props.theme.color[props.color || 'default']}1A;
                    }`;
			case 'link':
